...
public class FileParseUtil {
    ...
    @RequiresPermission(anyOf = {Manifest.permission.WRITE_EXTERNAL_STORAGE,Manifest.permission.READ_EXTERNAL_STORAGE})
    public static ByteBuffer parseBinFile2(@NonNull File file,int startAddr) throws Exception, IOException {
        if(file==null || !file.exists() || !file.isFile() ||(!file.getName().endsWith("bin") && !file.getName().endsWith("BIN"))){
            throw new Exception("this file don't exist or is invalid");
        }
        long length = file.length();
    ...
    public static ByteBuffer parseHexFile(@NonNull File file) throws Exception {
        ...
        try {
            ...
            while ((hexStr=reader.readLine())!=null){
                ...
                if(type==0x00){//本行的数据类型为“数据记录”
                    ...
                    //FcLog.d("readData:" +bytesToHexString(fileStruct.getData()) );
                    list.add(hexRec);
                    totalLen+=hexRec.getData().length;
                    //FcLog.d("total size: "+totalLen);
                    ...
                    FcLog.d("Start Segment Address record: ignored");
                    ...
                    FcLog.d("Start Linear Address record: ignored");
                }else {
        ...
        FcLog.d("final total size: "+totalLen);
        FcLog.d("min address: "+minAddr);
        FcLog.d("max address: "+maxAddr);
        ...
        if(realSize!=totalLen){
            FcLog.d("hex文件地址不连续："+"realSize->"+realSize+" totalLen"+totalLen);
        }
