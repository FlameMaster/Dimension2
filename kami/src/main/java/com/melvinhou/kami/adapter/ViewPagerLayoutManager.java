...
public class ViewPagerLayoutManager extends LinearLayoutManager {
    ...
    public void onScrollStateChanged(int state) {
        switch (state) {
            case RecyclerView.SCROLL_STATE_IDLE:
                ...
                if (mViewPagerListener != null && getChildCount() == 1) {
                    mViewPagerListener.onPageSelected(positionIdle,positionIdle == getItemCount() - 1, mIsLeftScroll);
                }
                break;
        }
    }
    ...
    private RecyclerView.OnChildAttachStateChangeListener mChildAttachStateChangeListener = new RecyclerView.OnChildAttachStateChangeListener() {
        ...
        public void onChildViewAttachedToWindow(View view) {
            if (mViewPagerListener != null && getChildCount() == 1) {
                mViewPagerListener.onInitComplete();
            }
        }
        ...
        public void onChildViewDetachedFromWindow(View view) {
            if (mDrift >= 0){
                if (mViewPagerListener != null) mViewPagerListener.onPageRelease(true,getPosition(view));
            }else {
                if (mViewPagerListener != null) mViewPagerListener.onPageRelease(false,getPosition(view));
            }
