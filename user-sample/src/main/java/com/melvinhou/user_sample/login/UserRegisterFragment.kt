...
import com.melvinhou.user_sample.databinding.FragmentUserRegisterBinding
...
class UserRegisterFragment :
    KindFragment<FragmentUserRegisterBinding, LoginModel>() {
    ...
    override fun initListener() {
        ...
        mBinding.ivPasswordRepeatCleal.setOnClickListener {
            mBinding.etPasswordRepeat.text.clear()
        }
    ...
    private fun userRegister() {
        ...
        val password2 = mBinding.etPasswordRepeat.text.toString()
        if (password!=password2) {
            FcUtils.showToast("两次密码不一致")
            return
        }
        if (mModel.checkParameter(mobile = mobile, authCode = authCode, password = password)) {
            mModel.register("86", mobile, authCode, password) {data->
                data?.let {
                    onSuceessLogin(it)
                }
            }
