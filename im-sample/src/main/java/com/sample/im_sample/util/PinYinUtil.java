package com.sample.im_sample.util;
...
public class PinYinUtil {
    ...
    public static char getPinyin(char chinese) {
        char initial = '#';
        //用来设置转化的拼音的大小写，或者声调
        HanyuPinyinOutputFormat format = new HanyuPinyinOutputFormat();
        format.setCaseType(HanyuPinyinCaseType.UPPERCASE);//设置转化的拼音是大写字母
        ...
        if (chinese > 127) {
            ...
            try {
                //由于多音字的存在，比如单  dan shan,
                String[] pinyinArr = PinyinHelper.toHanyuPinyinStringArray(chinese, format);
                if (pinyinArr != null) {
                    initial =  pinyinArr[0].toCharArray()[0];//此处即使有多音字，那么也只能取第一个拼音
                } else {
                    //说明没有找到对应的拼音，汉字有问题，或者可能不是汉字，则忽略
                }
            } catch (BadHanyuPinyinOutputFormatCombination e) {
